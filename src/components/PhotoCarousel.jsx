...
const SLIDES = [...PHOTO_SRCS, ...PHOTO_SRCS].map((src, i) => ({
  ...
  label: ['Captação Premium', 'Identidade Visual', 'Branding Editorial', 'Presença Digital'][i % 4],
}))
  ...
  return (
    ...
    >
      ...
      <div className="container">
        ...
        >
          <p className="section-label" style={{ color: 'var(--gold)' }}>Nossa Produção</p>
          <h2 style={{
            fontFamily: 'var(--font-serif)', fontWeight: 300,
            fontSize: 'clamp(32px, 4.5vw, 58px)', lineHeight: 1.06,
            color: 'var(--cream)', marginBottom: '18px',
          }}>
            Estética que converte<br />
            <em style={{ fontStyle: 'italic', color: 'var(--gold-light)' }}>e posiciona.</em>
          </h2>
          <p style={{ color: 'rgba(243,235,226,0.42)', fontSize: 'clamp(14px,1.4vw,16px)', lineHeight: 1.7, fontWeight: 300 }}>
      ...
      >
        <div style={{
          ...
          {SLIDES.map((slide, i) => {
            ...
            const raw     = i - active
            const wrapped = ((raw + SLIDES.length / 2 + SLIDES.length) % SLIDES.length) - SLIDES.length / 2
            const isAct   = wrapped === 0
            const isAdj   = Math.abs(wrapped) === 1

            const tX      = wrapped * CARD_GAP
            const tScale  = isAct ? 1 : isAdj ? 0.80 : 0.65
            const tOp     = isAct ? 1 : isAdj ? 0.40 : 0
            ...
            return (
              <motion.div
                ...
                transition={phase === 'entering'
                  ...
                  ? { type: 'spring', stiffness: 48, damping: 13, delay: i * 0.09 }
              ...
              >
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    background: 'linear-gradient(to bottom, transparent 50%, rgba(8,2,4,0.7) 100%)',
                  }} />
                  ...
                  {[['top','left'],['top','right'],['bottom','left'],['bottom','right']].map(([v,h]) => (
                    <div key={v+h} style={{
                      ...
                      borderTop:    v === 'top'    ? '1px solid rgba(184,147,90,0.4)' : 'none',
                      borderBottom: v === 'bottom' ? '1px solid rgba(184,147,90,0.4)' : 'none',
                      borderLeft:   h === 'left'   ? '1px solid rgba(184,147,90,0.4)' : 'none',
                      borderRight:  h === 'right'  ? '1px solid rgba(184,147,90,0.4)' : 'none',
                      pointerEvents: 'none',
                    }} />
                  ))}
                  ...
                  {isAct && (
                    <div style={{
                      ...
                      <span style={{
                        ...
                        color: 'rgba(243,235,226,0.55)', fontWeight: 600,
                      }}>
                        {slide.label}
                      </span>
                    </div>
        ...
        {['prev', 'next'].map(dir => (
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(184,147,90,0.55)'; e.currentTarget.style.color = 'var(--gold)' }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(243,235,226,0.1)'; e.currentTarget.style.color = 'rgba(243,235,226,0.45)' }}
          >
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '36px' }}>
        {SLIDES.map((_, i) => (
          <button
            ...
            onClick={() => goTo(i)}
            style={{
              width: i === active ? 26 : 5, height: 5, padding: 0, border: 'none',
              background: i === active ? 'var(--gold)' : 'rgba(243,235,226,0.12)',
      ...
      <div className="container" style={{ marginTop: '60px' }}>
        ...
        >
          <div style={{ flex: 1, height: '1px', background: 'rgba(243,235,226,0.06)' }} />
          <p style={{ color: 'rgba(243,235,226,0.22)', fontSize: '10px', letterSpacing: '0.16em', textTransform: 'uppercase', whiteSpace: 'nowrap', margin: 0 }}>
